// Admin Dashboard - Main SPA Application Controller
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Navigation links
  const sidebarLinks = document.querySelectorAll("aside .sidebar a[data-view]");
  ...
  // ----------------------------------------------------
  // SIDEBAR & SPA NAVIGATION CONTROLLER
  // ----------------------------------------------------
  ...
  // ----------------------------------------------------
  // HEADER DROPDOWNS & GLOBAL SEARCH CONTROLLER
  // ----------------------------------------------------
  ...
  function renderDashboardOrders() {
    ...
    // Show first 6 recent orders
    store.orders.slice(0, 6).forEach((order) => {
      const tr = document.createElement("tr");
      tr.innerHTML = `
        <td><b>${order.productName}</b></td>
        <td>#${order.productNumber}</td>
        <td><span class="tag-badge ${order.paymentStatus === 'Paid' ? 'success' : order.paymentStatus === 'Refunded' ? 'danger' : 'warning'}">${order.paymentStatus}</span></td>
        <td class="${
          order.shipping === "Declined" ? "danger" : order.shipping === "Pending" ? "warning" : "success"
        }">${order.shipping}</td>
        <td>
          <button class="btn-sm-outline btn-order-details" data-num="${order.productNumber}">Details</button>
        </td>
      `;
  ...
  function renderUpdatesFeed() {
    ...
    store.updates.forEach((upd) => {
      ...
      div.innerHTML = `
        <div class="profile-photo"><img src="${upd.avatar}" alt="${upd.name}" /></div>
        <div class="message">
          <p><b>${upd.name}</b> ${upd.action}</p>
  ...
  if (dashDateInput) {
    ...
    dashDateInput.addEventListener("change", (e) => {
      showToast(`Dashboard stats updated for ${e.target.value}`, "success");
    });
  }

  // Clicking Show All Orders on Dashboard
  const linkShowAll = document.getElementById("link-show-all-orders");
  if (linkShowAll) {
    linkShowAll.addEventListener("click", (e) => {
      e.preventDefault();
      navigateToView("orders");
    });
  }

  // Insight cards modal trigger
  document.getElementById("card-total-sales")?.addEventListener("click", () => {
    openModal(`
      <h2>Total Sales Breakdown</h2>
      <p class="text-muted" style="margin-bottom:1rem;">Gross Revenue: $25,204 (81% of monthly target)</p>
      <div style="background:var(--color-background); padding:1rem; border-radius:1rem; margin-bottom:1rem;">
        <p>• Online Store Sales: <b>$18,450</b></p>
        <p>• Direct Offline Wholesale: <b>$6,754</b></p>
        <p>• Avg Order Price: <b>$420</b></p>
      </div>
      <button class="btn btn-primary" onclick="document.getElementById('global-modal').classList.add('hidden')">Close</button>
    `);
  });

  document.getElementById("card-total-expenses")?.addEventListener("click", () => {
    openModal(`
      <h2>Total Expenses Overview</h2>
      <p class="text-muted" style="margin-bottom:1rem;">Operational Expenditure: $21,200 (72% of budget used)</p>
      <div style="background:var(--color-background); padding:1rem; border-radius:1rem; margin-bottom:1rem;">
        <p>• Logistics & Shipping: <b>$9,400</b></p>
        <p>• Marketing & Ads: <b>$6,800</b></p>
        <p>• Inventory Replenishment: <b>$5,000</b></p>
      </div>
      <button class="btn btn-primary" onclick="document.getElementById('global-modal').classList.add('hidden')">Close</button>
    `);
  });

  document.getElementById("card-total-income")?.addEventListener("click", () => {
    openModal(`
      <h2>Total Income Analysis</h2>
      <p class="text-muted" style="margin-bottom:1rem;">Net Profit Margin: $36,564 (+44% YoY Growth)</p>
      <div style="background:var(--color-background); padding:1rem; border-radius:1rem; margin-bottom:1rem;">
        <p>• Net Operating Income: <b>$28,190</b></p>
        <p>• Licensing & Accessories: <b>$8,374</b></p>
      </div>
      <button class="btn btn-primary" onclick="document.getElementById('global-modal').classList.add('hidden')">Close</button>
    `);
  });
  ...
  function renderCustomersView() {
    ...
    filtered.forEach((cust) => {
      ...
      tr.innerHTML = `
        ...
        <td><span class="tag-badge ${cust.status === 'Active' ? 'success' : 'danger'}">${cust.status}</span></td>
        <td>
          <button class="btn-sm-outline btn-cust-view" data-id="${cust.id}">View</button>
  ...
  function openCustomerDetailsModal(id) {
    ...
    openModal(`
      <div style="text-align:center; margin-bottom:1rem;">
        ...
        <h2>${cust.name}</h2>
        <span class="tag-badge ${cust.status === 'Active' ? 'success' : 'danger'}">${cust.status}</span>
      </div>
      <div style="background:var(--color-background); padding:1rem; border-radius:1rem; margin-bottom:1rem;">
        ...
        <p>• <b>Customer Since:</b> ${cust.joinedDate}</p>
      </div>
      <div class="form-actions-flex">
        <button class="btn btn-outline" onclick="document.getElementById('global-modal').classList.add('hidden')">Close</button>
        <button class="btn btn-primary" id="btn-toggle-cust-status">${cust.status === 'Active' ? 'Deactivate' : 'Activate'} Customer</button>
      </div>
    `);
  ...
  function renderOrdersView() {
    ...
    list.forEach((ord) => {
      ...
      tr.innerHTML = `
        ...
        <td>${ord.customer}</td>
        <td><b>$${ord.price}</b></td>
        <td><span class="tag-badge ${ord.paymentStatus === 'Paid' ? 'success' : ord.paymentStatus === 'Refunded' ? 'danger' : 'warning'}">${ord.paymentStatus}</span></td>
        <td class="${ord.shipping === 'Declined' ? 'danger' : ord.shipping === 'Pending' ? 'warning' : 'success'}">${ord.shipping}</td>
        <td>${ord.date}</td>
        <td>
          <button class="btn-sm-outline btn-ord-view" data-num="${ord.productNumber}">View Receipt</button>
    ...
    tbody.querySelectorAll(".btn-ord-del").forEach((btn) => {
      btn.addEventListener("click", () => {
        const num = btn.getAttribute("data-num");
        if (confirm(`Are you sure you want to delete order #${num}?`)) {
  ...
  function triggerCreateOrderModal() {
    ...
    document.getElementById("form-modal-create-ord")?.addEventListener("submit", (e) => {
      ...
      renderOrdersView();
      renderDashboardOrders();
      showToast(`New Order #${num} created successfully!`, "success");
    });
  }

  document.getElementById("btn-quick-new-order")?.addEventListener("click", triggerCreateOrderModal);
  ...
  function openOrderDetailsModal(num) {
    ...
    openModal(`
      ...
      <div style="background:var(--color-background); padding:1.2rem; border-radius:1rem; margin-bottom:1.2rem;">
        ...
        <div style="display:flex; justify-content:space-between; margin-bottom:0.5rem;">
          <span>Order Date:</span> <b>${ord.date}</b>
        </div>
        <div style="display:flex; justify-content:space-between; margin-bottom:0.5rem;">
          <span>Total Amount:</span> <b style="font-size:1.1rem; color:var(--color-primary);">$${ord.price}</b>
        </div>
      </div>

      <div class="form-group">
        <label>Update Shipping Fulfillment</label>
    ...
    document.getElementById("btn-save-ord-status")?.addEventListener("click", () => {
      ...
      renderOrdersView();
      renderDashboardOrders();
      showToast(`Order #${num} status updated`, "success");
    });
  }

  // 4. ANALYTICS VIEW PERIOD CONTROLLER
  document.querySelectorAll(".pill-toggle-group .pill-btn").forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      showToast(`Analytics view updated to ${period}`, "info");
    });
  });
  ...
  function renderMessagesInbox() {
    ...
    filtered.forEach((msg) => {
      ...
      item.innerHTML = `
        <div class="profile-photo"><img src="${msg.avatar}" alt="${msg.sender}" /></div>
    ...
    if (wrapper) {
      ...
      wrapper.innerHTML = `
        <div class="message-detail-header">
          <div class="message-author-flex">
            <div class="profile-photo" style="width:3.2rem; height:3.2rem;"><img src="${msg.avatar}" alt="${msg.sender}" /></div>
            <div>
        ...
        <div style="margin-top:2rem; padding-top:1rem; border-top:1px solid var(--color-light);">
          <h4>Quick Reply</h4>
          <textarea id="reply-text-box" rows="3" placeholder="Type your reply to ${msg.sender}..." style="width:100%; margin:0.6rem 0; padding:0.8rem; border-radius:0.6rem; border:1px solid var(--color-light); font-family:inherit;"></textarea>
  ...
  document.getElementById("btn-compose-msg")?.addEventListener("click", () => {
    openModal(`
      ...
      <form id="form-compose-msg" style="margin-top:1rem;">
        ...
        <div class="form-group">
          <label>Subject *</label>
          <input type="text" id="m-msg-subject" required placeholder="Enter message subject" />
        </div>
        <div class="form-group">
  ...
  function renderProductsView() {
    ...
    if (isGridView) {
        ...
        card.innerHTML = `
          ...
          <div class="product-card-body">
            <div style="display:flex; justify-content:space-between; align-items:center;">
              <span class="tag-badge ${prod.status === 'In Stock' ? 'success' : prod.status === 'Low Stock' ? 'warning' : 'danger'}">${prod.status}</span>
            ...
            <div class="product-card-footer">
              ...
              <div>
                <button class="btn-sm-outline btn-edit-prod" data-id="${prod.id}">Edit</button>
      ...
      container.className = "recent-orders";
        ...
        <tbody>
          ${filtered.map((prod) => `
            <tr>
              ...
              <td><span class="tag-badge ${prod.status === 'In Stock' ? 'success' : prod.status === 'Low Stock' ? 'warning' : 'danger'}">${prod.status}</span></td>
              <td>
                <button class="btn-sm-outline btn-edit-prod" data-id="${prod.id}">Edit</button>
                <button class="btn-danger-sm btn-del-prod" data-id="${prod.id}" style="margin-left:0.3rem;">Delete</button>
              </td>
  ...
  document.getElementById("btn-create-product-view")?.addEventListener("click", () => {
  ...
  if (formAddProdMain) {
    formAddProdMain.addEventListener("submit", (e) => {
      ...
      const newProd = {
        ...
        image: image,
        description: desc || "New product added to inventory."
      };
  ...
  document.getElementById("form-profile-settings")?.addEventListener("submit", (e) => {
    ...
    // Update Header Info
    document.getElementById("header-user-name").textContent = store.settings.fullName.split(" ")[0];
  ...
  // 10. LOGOUT & LOCKSCREEN CONTROLLER
  function handleLogoutPrompt() {
    openModal(`
      <div style="text-align:center; padding:1rem 0;">
        ...
        <p class="text-muted" style="margin:0.5rem 0 1.4rem;">Are you sure you want to sign out or lock your dashboard session?</p>
      ...
      openModal(`
        <div style="text-align:center; padding:1.5rem 0;">
          <div class="profile-photo" style="width:5rem; height:5rem; margin:0 auto 1rem;">
            <img src="${store.settings.avatar || 'images/image2.png'}" alt="Admin Avatar" />
          </div>
          <h2>Session Locked</h2>
          <p class="text-muted" style="margin-bottom:1rem;">Enter PIN or click below to unlock session</p>
          <input type="password" value="123456" style="width:80%; text-align:center; letter-spacing:4px; font-size:1.4rem; margin-bottom:1rem; padding:0.6rem; border-radius:0.6rem; border:1px solid var(--color-light);" />
          <br/>
